...
function EditProfile() {
  ...
  async function save(data) {
    ...
    let res = location.state ? await updateUser(location.state, formData) : await createUser(formData);
    if (res) {
      if (res.statusCode === 200 && res.taskStatus) {
        Swal.fire({
          icon: 'success',
          ...
          {({ values, errors, touched, setFieldValue }) => (
            <Form>
              <div className="row d-flex justify-content-center" >
                <div className='card align-items-center' style={{border: "1px solid gray", width: "1200px", height:"1050px",backgroundColor:"lightblue"}}>
